...
data = pd.read_csv(r"21_05_2023//morph_order_by_entropy_and_verb_perc2023_05_21.csv", encoding="ISO-8859-1")
...
def save_annotations(user, annotations):
    ...
    annotations.to_csv(file_name, index=False, columns=["Sentence", "Tag according to dimension",
                                                        "Creativity Scale", "Notes on relevant dimension",
                                                        "Notes"])
...
def load_annotations(user):
    ...
    if "Tag according to dimension" not in annotations.columns:
        annotations["Tag according to dimension"] = ""

    if "Notes on relevant dimension" not in annotations.columns:
        annotations["Notes on relevant dimension"] = ""

    if "Notes" not in annotations.columns:
        annotations["Notes"] = ""

    if "Creativity Scale" not in annotations.columns:
        annotations["Creativity Scale"] = ""

    merged = pd.merge(data, annotations[["Sentence", "Tag according to dimension", "Creativity Scale",
                                         "Notes on relevant dimension", "Notes"]],
                      on="Sentence", how="outer")
    merged["Tag according to dimension"].fillna("", inplace=True)
    merged["Notes on relevant dimension"].fillna("", inplace=True)
    merged["Notes"].fillna("", inplace=True)
...
# Main app
def main():
    global user_annotations  # Make user_annotations accessible across the app

    # Load user annotations for each user
    users = ["Nurit", "Ittamar", "Gabi", "Shahar"]
    user_annotations = {user: load_annotations(user) for user in users}
    ...
    if not view_all_annotations:
        # Get user's annotations from the dictionary
        user_annotations = user_annotations[user]
        ...
        for index, row in user_annotations.iterrows():
            ...
            for token in doc:
                if token.i == row["index of verb"]:
                    ...
                    new_sentence = sentence.replace(token.text, f"<b>{token.text}</b>")
            ...
            if expand_button:
                ...
                if f"{user}_expanded_{index}" not in st.session_state:
                    st.session_state[f"{user}_expanded_{index}"] = {"extra_data": True, "dependency_tree": False}
                else:
                    st.session_state[f"{user}_expanded_{index}"]["extra_data"] = not st.session_state[f"{user}_expanded_{index}"]["extra_data"]

            # Display extra data if expanded for the current user
            if f"{user}_expanded_{index}" in st.session_state and st.session_state[f"{user}_expanded_{index}"]["extra_data"]:
            ...
            dependency_tree_button = st.button("Show Dependency Tree", key=f"{user}_dependency_tree_button_{index}")
            if dependency_tree_button:
                # Toggle visibility of the dependency tree for the current user only
                if f"{user}_expanded_{index}" not in st.session_state:
                    st.session_state[f"{user}_expanded_{index}"] = {"extra_data": False, "dependency_tree": True}
                else:
                    st.session_state[f"{user}_expanded_{index}"]["dependency_tree"] = not st.session_state[f"{user}_expanded_{index}"]["dependency_tree"]

            # Display dependency tree if expanded for the current user
            if f"{user}_expanded_{index}" in st.session_state and st.session_state[f"{user}_expanded_{index}"]["dependency_tree"]:
            ...
            annotation = st.selectbox("Tag according to dimension", options=[
                "ordinary", "creative", "spelling variant", "wrong lemma", "not a verb",
                "algorithm error", "not English"
            ...
            scale = st.selectbox("Creativity Scale (1-5)", options=[1, 2, 3, 4, 5], key=f"{user}_creativity_scale_{index}")
